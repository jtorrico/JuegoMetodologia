...
public class InstanciaProjectilEscopeta : MonoBehaviour {

    //Variables de cantidad actual de balas, cantidad maxima, delay de recarga, control y animaciones.
    public int cantidadBalasEscopeta;
    public int cantidadMaximaBalasEscopeta = 8;
    public float retardoRecarga = 1.5f;
    ...
    public float retardoDisparo = 0.35f;
	...
	void Start ()
    {
        ...
        cantidadBalasEscopeta = cantidadMaximaBalasEscopeta;
        tipoMunicion();
	}
    ...
    void Update ()
    {
        ...
        if (cantidadBalasEscopeta == 0 || Input.GetButton("Recarga"))
        {
    ...
    void SetMunicionEscopeta()
    {
        ...
        municionEscopetaDisplay.text = cantidadBalasEscopeta.ToString();
    }
    ...
    //Por cada disparo, se disminuye la cantidad de balas actuales y se actualiza el HUD, se reinicia el delay para evitar lo mencionado anteriormente, y se instancia
    //cada projectil y la reproduccion de la animacion, la unica diferencia entre el otro script es que en este script no hay reproduccion de sonido, ya que si lo tuviera,
    //se reproduciria el mismo sonido 4 veces al mismo tiempo.
    ...
    void dispararEscopeta()
    {
        ...
        if (Input.GetButton("Disparo1") && tiempoEnFrio <= 0)
        {
            //Shoot!
            Debug.Log("Pew!");
            cantidadBalasEscopeta -= 1;
            SetMunicionEscopeta();
            tiempoEnFrio = retardoDisparo;

            AnimacionDisparoEscopeta.SetBool("Disparo", true);
